...
class PdfService
{
    ...
    const FORMATS = [
        ...
        '80mm' => ['width' => 80, 'height' => 200, 'unit' => 'mm'], // Ticket ancho
        '58mm' => ['width' => 58, 'height' => 150, 'unit' => 'mm'], // Ticket angosto
    ];
    ...
    {
        // Generar QR Code
        $qrData = $this->generateQRDataForDispatch($dispatchGuide);
        $qrBase64 = $this->generateQRCode($qrData);
        ...
        return [
            ...
            'qr_code' => $qrBase64,
            'hash' => $dispatchGuide->codigo_hash ?? '',
        ];
    }
    ...
    protected function setPaperFormat(Dompdf $pdf, string $format): void
    {
        ...
        if ($formatLower === 'a4' || $formatLower === 'a5') {
            ...
            // Para formatos personalizados (80mm, 58mm)
    ...
    /**
     * Genera datos para QR de guía de remisión
     */
    protected function generateQRDataForDispatch($dispatchGuide): string
    {
        // Formato QR SUNAT para GRE: RUC|09|SERIE|NUMERO|FECHA_EMISION|
        $company = $dispatchGuide->company;

        $ruc = $company->ruc ?? '';
        $tipoDoc = '09'; // 09 = Guía de Remisión Electrónica
        $serie = $dispatchGuide->serie ?? '';
        $numero = str_pad($dispatchGuide->correlativo ?? '', 8, '0', STR_PAD_LEFT);
        $fechaEmision = $dispatchGuide->fecha_emision ? $dispatchGuide->fecha_emision->format('Y-m-d') : date('Y-m-d');

        return "{$ruc}|{$tipoDoc}|{$serie}|{$numero}|{$fechaEmision}|";
    }
     ...
     */
    public function generateNotaVentaPdf($notaVenta, string $format = 'A4'): string
    {
        $data = $this->prepareNotaVentaData($notaVenta);
        $data['format'] = $format;

        $template = $this->getTemplate('nota-venta', $format);
        $html = View::make($template, $data)->render();
    ...
    /**
     * Preparar datos para PDF de Nota de Venta
     */
    protected function prepareNotaVentaData($notaVenta): array
    {
        $company = $notaVenta->company;
        $client = $notaVenta->client;

        // Preparar datos del cliente
        $clientData = [
            'tipo_documento' => $client->tipo_documento ?? '0',
        ...
        // Total en letras
        $totalEnLetras = $this->numeroALetras(
            $notaVenta->mto_imp_venta,
            $notaVenta->moneda
        );

        // Código QR
        $qrCode = $this->generateQRCodeForNotaVenta($notaVenta);

        // Calcular totales usando el método estándar
        $totales = $this->calculateInvoiceTotals($notaVenta);
        ...
        return [
            ...
            'totales' => $totales,
            'tipo_documento_nombre' => 'NOTA DE VENTA',
            'fecha_emision' => $notaVenta->fecha_emision->format('d/m/Y'),
            'total_en_letras' => $totalEnLetras,
            'qr_code' => $qrCode,
            'hash' => $notaVenta->codigo_hash,
        ];
    }

    /**
     * Generar QR para Nota de Venta (usa el método estándar)
     */
    protected function generateQRCodeForNotaVenta(NotaVenta $notaVenta): ?string
    {
        try {
            // Usar el método estándar de generación de QR
            $qrData = $this->generateQRData($notaVenta);
            return $this->generateQRCode($qrData);

        } catch (\Exception $e) {
            Log::warning('Error generando QR para Nota de Venta', [
